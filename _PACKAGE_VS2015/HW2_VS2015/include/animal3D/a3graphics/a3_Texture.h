...
//-----------------------------------------------------------------------------
	...
	struct a3_Texture
	{
		...
		unsigned int channels, bytes;
		unsigned short internalFormat, internalType;
	};
	...
	{
		unsigned short internalFormat;
		unsigned short internalFormatBits;
		unsigned short internalDataType;
		unsigned char channelsPerPixel;
	...
	enum a3_TexturePixelType
	{
		...
		a3tex_depth32,			// 32-bit depth (int)
...
//-----------------------------------------------------------------------------
	...
	int a3textureCreatePixelFormatDescriptor(a3_TexturePixelFormatDescriptor *pixelFormat_out, const a3_TexturePixelType pixelType);
...
//-----------------------------------------------------------------------------
	...
	int a3textureInitializeImageLibrary();
	...
	int a3textureCreateFromFile(a3_Texture *texture_out, const char *filePath);
	...
	//	return: -1 if invalid params or already initialized
	int a3textureCreateFromData(a3_Texture *texture_out, const a3_TexturePixelFormatDescriptor *pixelFormat, const unsigned int width, const unsigned int height, const void *data_opt, int dataFlipped);
	...
	int a3textureReplaceData(const a3_Texture *texture, const unsigned int offsetWidth, const unsigned int offsetHeight, const unsigned int replaceWidth, const unsigned int replaceHeight, const void *data_opt, int dataFlipped);
	...
	int a3textureActivate(const a3_Texture *texture, const a3_TextureUnit unit);

	// A3: Deactivate texture on specified slot.
	//	param unit: texture unit to unbind texture from
	//	return: 0
	int a3textureDeactivate(const a3_TextureUnit unit);

	// A3: Reference texture.
	//	param texture: non-null pointer to initialized texture
	//	return: new reference count if success
	//	return: -1 if invalid params or not initialized
	int a3textureReference(a3_Texture *texture);
...
//-----------------------------------------------------------------------------
	...
	//	return: 0
	int a3textureDefaultSettings();

	// A3: Change filter settings for active texture.
	//	param filterOption: which filter mode to use
	//	return: 1
	int a3textureChangeFilterMode(const a3_TextureFilterOption filterOption);
	...
	//	return: 1
	int a3textureChangeRepeatMode(const a3_TextureRepeatOption repeatOptionHoriz, const a3_TextureRepeatOption repeatOptionVert);
