...
inline int a3vertexArrayGetCurrentCount(const a3_VertexArrayDescriptor *vertexArray)
{
...
inline int a3vertexArrayStore(a3_VertexArrayDescriptor *vertexArray, const a3_VertexAttributeDataDescriptor *attribRawData, const unsigned int vertexCount, unsigned int *vertexBufferOffset_out_opt, unsigned int *firstVertex_out_opt)
{
	if (vertexArray && vertexArray->handle->handle)
	{
		int ret = a3vertexBufferStore(vertexArray->vertexBuffer, vertexArray->vertexFormat, attribRawData, vertexCount, vertexBufferOffset_out_opt);
		if (ret > 0)
		{
...
inline int a3vertexArrayReferenceDescriptor(a3_VertexArrayDescriptor *vertexArray)
{
	if (vertexArray)
	{
...
inline int a3vertexArrayReleaseDescriptor(a3_VertexArrayDescriptor *vertexArray)
{
	if (vertexArray)
	{
		int ret;
